...
impl PersistentModifierResponses {
    ...
    fn contains(&self, perk: Perks) -> bool {
        self.sbr.contains_key(&perk)
            || self.dmr.contains_key(&perk)
            || self.hmr.contains_key(&perk)
            || self.rmr.contains_key(&perk)
            || self.rsmr.contains_key(&perk)
            || self.fmr.contains_key(&perk)
            || self.flmr.contains_key(&perk)
            || self.edr.contains_key(&perk)
            || self.rr.contains_key(&perk)
            || self.vmr.contains_key(&perk)
            || self.epr.contains_key(&perk)
            || self.mmr.contains_key(&perk)
            || self.imr.contains_key(&perk)
    }
}

pub fn is_perk_implemented(perk: Perks) -> bool {
    map_perks();
    PERK_FUNC_MAP.with(|map| map.borrow().contains(perk))
}
