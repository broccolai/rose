...
#[test]
fn test_target_health_ttk_inputs_are_monotonic() {
    setup_pulse();
    PERS_DATA.with(|perm_data| {
        let weapon = perm_data.borrow().weapon.clone();
        let baseline = crate::weapons::ttk_calc::calc_ttk_at_health(&weapon, 230.0, 0.0, 1.0);
        let shielded = crate::weapons::ttk_calc::calc_ttk_at_health(&weapon, 230.0, 10.0, 1.0);
        let weapon_bonus = crate::weapons::ttk_calc::calc_ttk_at_health(&weapon, 230.0, 0.0, 1.06);

        assert_eq!(shielded.target_health, 240.0);
        assert!(shielded.optimal_ttk.time_taken >= baseline.optimal_ttk.time_taken);
        assert!(shielded.body_ttk.time_taken >= baseline.body_ttk.time_taken);
        assert!(weapon_bonus.optimal_ttk.time_taken <= baseline.optimal_ttk.time_taken);
        assert!(weapon_bonus.body_ttk.time_taken <= baseline.body_ttk.time_taken);
    });
}
...
fn test_phase_mag() {
    ...
    assert_eq!(
        f64::trunc(lw_data.pvp_impact_damage * 1000.0) / 1000.0,
        p_data.pvp_impact_damage
    );
}
