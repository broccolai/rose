...
pub fn get_metadata() -> Result<JsMetaData, JsValue> {
    let metadata = JsMetaData {
        api_timestamp: "2026-05-19",
        api_version: env!("CARGO_PKG_VERSION"),
        api_commit: "dcd0c7d302b056ffbb532edcb711d724a17ca8c1",
        api_branch: "rose-vendor",
    };
    Ok(metadata)
}
...
pub fn set_weapon(
    ...
    let configured = PERS_DATA.with(|perm_data| {
...
#[wasm_bindgen(js_name = "isTraitSupported")]
pub fn is_trait_supported(perk_hash: u32) -> bool {
    let normalized = perks::enhanced_check(perk_hash).0;
    let perk = Perks::from(normalized);
    perk != Perks::Ignore && perks::is_perk_implemented(perk)
}
...
#[wasm_bindgen(js_name = "getWeaponTtkAtHealth")]
pub fn get_weapon_ttk_at_health(
    target_health: f64,
    overshield: f64,
    damage_scalar: f64,
    ...
    let weapon = PERS_DATA.with(|perm_data| perm_data.borrow().weapon.clone());
    let ttk_data =
        weapons::ttk_calc::calc_ttk_at_health(&weapon, target_health, overshield, damage_scalar);
    Ok(serde_wasm_bindgen::to_value(&ttk_data).unwrap())
}
