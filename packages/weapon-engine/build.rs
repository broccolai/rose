...
impl CachedBuildData {
    ...
    fn get_timestamp(&self, formula: &impl Hash) -> u64 {
        let hash = calculate_hash(&formula);
        self.perk_timestamps.get(&hash).copied().unwrap_or(0)
    }
}
...
fn main() {
    println!("cargo:rerun-if-changed=build_resources/cached_build.ron");
    println!("cargo:rerun-if-changed=build_resources/weapon_formulas.json");
    ...
    let build_cache_path = std::path::Path::new(&std::env::var("CARGO_MANIFEST_DIR").unwrap())
        .join("build_resources/cached_build.ron");
    let cached_data: CachedBuildData;
    if !build_cache_path.exists() {
        println!("cargo:warning=no cached build file found");
        cached_data = CachedBuildData::default();
    } else {
        match std::fs::File::open(build_cache_path) {
            Ok(file) => {
                cached_data = ron::de::from_reader(file).unwrap_or_else(|error| {
                    panic!("cargo:warning=error reading cached build file: {error}")
                });
            }
            Err(error) => {
                println!("cargo:warning=error opening cached build file: {error}");
                cached_data = CachedBuildData::default();
            }
        }
    }

    construct_enhance_perk_mapping(&mut formula_file, &cached_data);
    construct_weapon_formulas(&mut formula_file, &cached_data);
}

//fn set_data(val: Value, weapon_def: Value, weapon_hash: u32, weapon_id: String) {]

fn construct_weapon_formulas(formula_file: &mut File, cached: &CachedBuildData) {
...
fn construct_enhance_perk_mapping(formula_file: &mut File, cached: &CachedBuildData) {
    assert!(cached.has_data(), "vendored perk mapping cache is missing");
    let mut perk_mappings = cached.dim_perk_mappings.clone();
    perk_mappings.extend_from_slice(&cached.procedural_intrinsic_mappings);
    write_variable(
        formula_file,
